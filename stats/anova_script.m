function [RMAOV31] = anova_script(x,alpha)
...
mnum = max(x);                                                              % estimates the maximum number of all variables in (x)
name_var = {'indep. variable 1 (IV1)','indep. variable 2 (IV2)', ...
    'indep. variable 3 (IV3)'};                                             % for output purposes

%% Start with the estimation
disp('------------------------------------------------------------------');
for n = 1:3 % loop through three independent variables
    fprintf('The number of %s levels are:%2i\n', name_var{n}, mnum(1,n+1))
end
fprintf('There are %2i subjects in total\n\n', mnum(1,5));

corr_term   = (sum(x(:,1)))^2/length(x);                                    % correction term for the mean of the dependent variables
SStot       = sum(x(:,1).^2)-corr_term;                                     % Total sum of squares, corrected for the mean of the DV
dftot       = length(x(:,1))-1;                                             % Total degrees of freedom, that is all measurements - 1

%% Estimate the "subject" factor (Sfac), which is later used to obtain the SS of the residuals (errors).
sfac = [];
for subj = 1:mnum(1,5)
    eval(['sfac' num2str(subj) '=x((x(:,5)==subj), 1);']);
    eval(['xtemp =((sum(sfac' num2str(subj) ').^2)/length(sfac' num2str(subj) '));']);
    sfac = [sfac,xtemp]; %#ok<AGROW>
end
...
for i = 1:mnum(1,2)
    ...
    eval(['xtemp =((sum(A' num2str(i) ').^2)/length(A' num2str(i) '));']);
    A = [A,xtemp];
end;
SSA = sum(A)-corr_term;  %sum of squares for the IV1
v1 = mnum(1,2)-1;  %degrees of freedom for the IV1
...
for j = 1:mnum(1,3)
    ...
    eval(['xtemp =((sum(B' num2str(j) ').^2)/length(B' num2str(j) '));']);
    B =[B,xtemp];
end;
SSB = sum(B)-corr_term;  %sum of squares for the IV2
v2 = mnum(1,3)-1;  %degrees of freedom for the IV2
...
for i = 1:mnum(1,2)
    for j = 1:mnum(1,3)
        ...
        eval(['xtemp =((sum(AB' num2str(i) num2str(j) ').^2)/length(AB' num2str(i) num2str(j) '));']);
        AB = [AB,xtemp];
    end
end
SSAB = sum(AB)-sum(A)-sum(B)+corr_term;  %sum of squares of the IV1xIV2
v3 = v1*v2;  %degrees of freedom of the IV1xIV2
MSAB = SSAB/v3;  %mean square for the IV1xIV2

%procedure related to the IV1-IV2-error.
SSEAB = sum(sfac)-sum(AB);  %sum of squares of the IV1-IV2-error
v4 = mnum(1,5)-(mnum(1,2)*mnum(1,3));  %degrees of freedom of the IV1-IV2-error
...
for k = 1:mnum(1,4)
    ...
    eval(['xtemp =((sum(C' num2str(k) ').^2)/length(C' num2str(k) '));']);
    C =[C,xtemp];
end;
SSC = sum(C)-corr_term;  %sum of squares for the IV2
v5 = mnum(1,4)-1;  %degrees of freedom for the IV2
...
for i = 1:mnum(1,2)
    for k = 1:mnum(1,4)
        ...
        eval(['xtemp =((sum(AC' num2str(i) num2str(k) ').^2)/length(AC' num2str(i) num2str(k) '));']);
        AC = [AC,xtemp];
    end;
end;
SSAC = sum(AC)-sum(A)-sum(C)+corr_term;  %sum of squares of the IV1xIV3
...
for j = 1:mnum(1,3)
    for k = 1:mnum(1,4)
        ...
        eval(['xtemp =((sum(BC' num2str(j) num2str(k) ').^2)/length(BC' num2str(j) num2str(k) '));']);
        BC = [BC,xtemp];
    end;
end;
SSBC = sum(BC)-sum(B)-sum(C)+corr_term;  %sum of squares of the IV2xIV3
...
for i = 1:mnum(1,2)
    for j = 1:mnum(1,3)
        for k = 1:mnum(1,4)
            ...
            eval(['xtemp =((sum(AB' num2str(i) num2str(j) num2str(k) ').^2)/length(AB' num2str(i) num2str(j) num2str(k) '));']);
            ABC = [ABC,xtemp];
        end;
    end;
end;
SSABC = sum(ABC)+sum(A)+sum(B)+sum(C)-sum(AB)-sum(AC)-sum(BC)-corr_term;  %sum of squares of the IV1xIV2xIV3
...
for k = 1:mnum(1,4)
    for l = 1:mnum(1,5)
        ...
        eval(['xtemp =((sum(IV3S' num2str(k) num2str(l) ').^2)/length(IV3S' num2str(k) num2str(l) '));']);
        EIV3 = [EIV3,xtemp];
    end;
end;
SSEC = sum(EIV3)-sum(sfac)-sum(ABC)+sum(AB);  %sum of squares of the IV3-error
