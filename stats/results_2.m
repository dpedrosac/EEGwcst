...
excl_subj = [ 16, 21 ];                                                     % excluded subjects due to technical reasons
...
%% ET-patients vs control subjects shift trials
avg_all = avg; avg_shiftALC=cell(1,2);
for k = 1:2; avg_shiftALC{k} = avg_all(idx_group{k}); end                            % separate average in groups
ch              = {'Fz', 'FCz', 'Cz', 'CPz', 'Pz', 'POz'};                         % channels of interest
toi = [-.15 1]; bsl = [-.15 0];
...
if ~exist(fullfile(ROOTDIR, 'data', 'avg_memoryALC_erp.mat'), 'file')
    estimate_erp_averages_generic([subj1, subj2], ROOTDIR, wdir, [110, 120], 'memoryALC')
end
