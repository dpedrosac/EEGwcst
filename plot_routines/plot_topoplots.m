...
cfgc.layout.pos(:,1) = cfgc.layout.pos(:,1)*1.3;                   % the next two lines shift the values so that
cfgc.layout.pos(:,2) = cfgc.layout.pos(:,2)*1.25;                    % the interpolation of results is minimised
...
for n = 1:2 % loop through both groups and estimate difference at the end
...
switch plot_stats
    case (1)
        ...
        cfgc.gridscale          = 40;                                       % number of interpolation points, the higher the better the resolution
        cfgc.interplimits       = 'head';                                   % interpolates data to the edges of the head
        cfgc.xlim               = [0.1 : 0.2 : 1.0];                                        % as only one frequency given, this value is 1. In cases of frequency vector this enables topoplots at different FOI
        cfgc.baseline           = bsl;
        cfgc.baselinetype       = 'absolute'; %'relchange';
        cfgc.zlim               = [-3 3];
