...
for g = 1:2 % loop through both groups (1) ET-patients, (2) control subjects
    ...
    %%  Estimate the distinct errors available in the experiment %%
    ...
    for k = 1:numel(dattmp) % subj. per group for individual results
        p.update( k )
    ...
    errors = arrayfun(@(x) cat(2,errors{x}{:}), 1:2, 'Un', 0);              % concatenates errors from both groups to one structure
    parti = unique({errors{1}(:).code}, 'stable');                          % creates a list of all available participants, somehow redundant as the same as subj1/subj2
    
    %% Summarise different errors
    err_tbl{g} = [];                                           %#ok<*AGROW> % pre-allocates space for
    for k = 1:numel(parti) % loop through all participants
        idx_subj = parti{k};
        % following *arrayfun avoids for-loops for w/ALC - wo/ALC condition
        idx_all = arrayfun(@(x) find(strcmp({errors{x}(:).code}, ...
            idx_subj)), 1:2, 'Un', 0);
        
        err_tbl{g}(k,1:2) = cell2mat(arrayfun(@(x) ...                      % the first two columns provide the anticipations for the WO and the ALC condition
            numel(find(strcmp({errors{x}(idx_all{x}).value}, 'S 40'))), ... % this doesn't make particular sense as S40 was removed in rt_gen.m (cf. lines 96f.)
            1:2, 'Un', 0));
        % ==> Anticipation w/ and wo/Alcohol
        ...
        err_tbl{g}(k,3:4) = cell2mat(arrayfun(@(x) ...                      % the third and fourth columns provide the efficient error for the WO and the ALC condition
            ...
            1:2, 'Un', 0));
        % ==> Efficient errors w/ and wo/Alcohol
        
        err_tbl{g}(k,5:6) = cell2mat(arrayfun(@(x) ...                      % the fifth and sixth columns provide the memory error for the WO and the ALC condition
            ...
            1:2, 'Un', 0));
        % ==> Memory errors w/ and wo/Alcohol
        
        err_tbl{g}(k,7:8) = cell2mat(arrayfun(@(x) ...                      % the seventh and eighth columns provide the perseverative errors for the WO and the ALC condition
            numel(find(strcmp({errors{x}(idx_all{x}).value}, 'S 50') & ...
            (strcmp({errors{x}(idx_all{x}).prev}, 'S  3')))), ...
            1:2, 'Un', 0));
        % ==> Perseveration errors w/ and wo/Alcohol
        
        % Old versions of err_tbl
...
function table1 = create_tableOne(data, rt_all, err_tbl)
